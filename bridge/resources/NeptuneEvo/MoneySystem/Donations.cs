    ...
    class Donations : Script
    {
        ...
        public static void LoadDonations()
        {
            Connection =
                ...
                $"Password={config.TryGet<string>("Password", "Rhjyjcnfh1337")};" +
                $"Database={config.TryGet<string>("Database", "ragempserver")};" +
                ...
                using (MySqlConnection connection = new MySqlConnection(Connection))
                {
                    ...
                    foreach (DataRow Row in result.Rows)
                    {
                        ...
                        try
                        {
                            ...
                            {
                                MySQL.Query($"update `accounts` set `GC`=`GC`+{reds} where `login`='{name}'");
                            }
                            else
                            {
                                lock (Main.Players)
                                {
                                    Main.Accounts[client].GC += reds;
                                }
                                NAPI.Task.Run(() =>
                                {
                                    try
                                    {
                                        ...
                                        Notify.Send(client, NotifyType.Success, NotifyPosition.BottomCenter, $"Вам пришли {reds} GC", 3000);
                                        Trigger.ClientEvent(client, "starset", Main.Accounts[client].GC);
                                    }
                                    catch { }
                                });
                            }
                    ...
                    case Type.Character:
                        {
                            if (acc.GC < 100)
                            {
                                Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "Недостаточно GC!", 3000);
                                return;
                            }
                            Main.Accounts[client].GC -= 100;
                    ...
                    case Type.Convert:
                        {
                            ...
                            if (Main.Accounts[client].GC < amount)
                            {
                                Notify.Send(client, NotifyType.Error, NotifyPosition.BottomCenter, "Недостаточно GC!", 3000);
                                return;
                            }
                            Main.Accounts[client].GC -= amount;
                    ...
                    case Type.Slot:
                        {
                            ...
                            Trigger.ClientEvent(client, "unlockSlot", Main.Accounts[client].GC);
                            MySQL.Query($"update `accounts` set `GC`={Main.Accounts[client].GC} where `login`='{Main.Accounts[client].Login}'");
                            return;
                        }
                }
                //Log.Write(Main.Players[client.Handle].Starbucks.ToString(), Logger.Type.Debug);
                MySQL.Query($"update `accounts` set `GC`={Main.Accounts[client].GC} where `login`='{Main.Accounts[client].Login}'");
                Trigger.ClientEvent(client, "redset", Main.Accounts[client].GC);
            }
