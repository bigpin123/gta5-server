...
namespace NeptuneEvo
{
    ...
    public class Main : Script
    {
        ...
        public static string Coder { get; } = "Classic RP";
        public static string Version { get; } = ""; // 2.2.4 r.i.p
        public static string Build { get; } = ""; // 1583 r.i.p
        // // // //
        public static string Full { get; } = $"{Coder} {Version} {Build}";
        ...
        public void onResourceStart()
        {
            try
            {
                NAPI.TextLabel.CreateTextLabel("~r~Bony", new Vector3(3367.203, 5185.236, 2.8402408), 5f, 0.3f, 0, new Color(255, 255, 255), true, 0);
                NAPI.TextLabel.CreateTextLabel("~r~Emma", new Vector3(3313.938, 5179.962, 20.81486), 5f, 0.3f, 0, new Color(255, 255, 255), true, 0);
                NAPI.TextLabel.CreateTextLabel("~r~Frank", new Vector3(1925.005, 4922.076, 49.27858), 5f, 0.3f, 0, new Color(255, 255, 255), true, 0);
                ...
                Timers.StartTask("savedb", 90000, () => saveDatabase());
            ...
            try
            {
                ...
                if (Players.ContainsKey(player))
                {
                    ...
                    try
                    {
                        ...
                        Jobs.spais.onPlayerDisconnected(player, type, reason);
        ...
        public void Event_OnPlayerConnected(Client player)
        {
            try
            {
                ...
                player.SendChatMessage($"!{{#ffb21d}} Добро пожаловать на сервер !{{#5b73fd}} Classic RP");
                player.SendChatMessage($"!{{#ffb21d}} Discord !{{#d86472}} discord.gg/......");
                player.SendChatMessage($"!{{#ffb21d}} Vkontakte !{{#d86472}} vk.com/......");
        ...
        public void ClientEvent_Spawn(Client player, int id)
        {
            ...
            try
            {
                ...
                Trigger.ClientEvent(player, "redset", Accounts[player].GC);
                    ...
                    if (Main.RestorePass.ContainsKey(client))
                    {
                        if (client.GetData("RealSocialClub") == Main.RestorePass[client].Item3)
                        {
                            if (Convert.ToInt32(loginorcode) == Main.RestorePass[client].Item1)
                            {
                                ...
                                MySQL.Query($"UPDATE `accounts` SET `password`='{newpas.ToString()}' WHERE `login`='{Main.RestorePass[client].Item2}' AND `socialclub`='{Main.RestorePass[client].Item3}'");
                ...
                switch (id)
                {
                   ...
                   case 9:
                    ...
                    case 29:
                        ...
                        return;
                    case 550:
                        Jobs.WorkManager.openspaisStart(player);
                        return;
                    case 551:
                        Jobs.spais.getspaisCar(player);
                        return;
                    case 30:
        ...
        private static void playedMinutesTrigger()
        {
            try
            {
                if (!oldconfig.SCLog)
                {
                    ...
                    if (now.Hour == 0)
                    {
                        if (now.Minute == 5) NAPI.Chat.SendChatMessageToAll("!{#DF5353}[AUTO RESTART] Дорогие игроки, в 01:20 произойдёт автоматический рестарт сервера.");
                        else if (now.Minute == 10) NAPI.Chat.SendChatMessageToAll("!{#DF5353}[AUTO RESTART] Дорогие игроки, напоминаем, что в 01:20 произойдёт автоматический рестарт сервера.");
                        else if (now.Minute == 15) NAPI.Chat.SendChatMessageToAll("!{#DF5353}[AUTO RESTART] Дорогие игроки, напоминаем, что в 01:20 произойдёт автоматический рестарт сервера.");
                ...
                try
                {
                    ...
                    foreach (Client player in Players.Keys.ToList())
                    {
                        try
                        {
                            ...
                            if (Players[player].LastHourMin < 1)
                            {
                                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Вы должны наиграть хотя бы 1 минуту, чтобы получить пейдей", 3000);
                                continue;
                            }
        ...
        public static void CMD_BUILD(Client client)
        {
            try
            {
                client.SendChatMessage($"!{{#ffffff}}Сервер !{{#5b73fd}}{Full}");
                client.SendChatMessage($"!{{#ffffff}}Спасибо что зашли на наш сервер ");
            }
            catch { }
        }
        ...
        public static async Task OpenPlayerMenu(Client player)
        {
            ...
            if (Main.Players[player].LVL < 1) //Промокод
            {
        ...
        private static void callback(Client player, Menu menu, Menu.Item item, string eventName, dynamic data)
        {
            MenuManager.Close(player);
            switch (item.ID)
            {
        ...
        private static Dictionary<string, Vector3> Points = new Dictionary<string, Vector3>()
        {
            ...
            { "Мексиканский картель", Fractions.Manager.FractionSpawns[12] },
