    ...
    class AlcoFabrication : Script
    {
        ...
        */
        ...
        private static Dictionary<int, Vector3> EnterAlcoShop = new Dictionary<int, Vector3>()
        {
            ...
            //{ 12, new Vector3(-564.5512, 275.6993, 81.98249) },Вход в клуб японцы текила
           // { 13, new Vector3(-430.1028, 261.2774, 81.88689) }, Вход в клуб Ванила
        };
        private static Dictionary<int, Vector3> ExitAlcoShop = new Dictionary<int, Vector3>()
        {
            { 10, new Vector3(-1387.458, -588.3003, 29.19951) },
            //{ 12, new Vector3(-564.487, 277.4747, 82.01633) }, Выход из клуба
        ...
        {
            { 10, new Vector3(-1404.037, -633.443, 27.55337) }, //Материалы клуба 
            { 11, new Vector3(141.3792, -1281.576, 28.2172) },
            { 12, new Vector3(-21.83, 216.02, 105.56) },
            { 13, new Vector3(-452.4567, 290.8813, 82.113) },
        };
        private static Dictionary<int, Vector3> BuyPoints = new Dictionary<int, Vector3>()
        {
            { 10, new Vector3(-1394.523, -602.7082, 29.19955) }, //Блип закупки алкаголя  - 1 ставить
            { 11, new Vector3(126.4378, -1282.892, 28.15849) },
            { 12, new Vector3(-22.65, 222.17, 95.70) },
        ...
        public void Event_ResourceStart()
        {
            try
            {
                ...
                NAPI.Blip.CreateBlip(93, new Vector3(-1388.761, -586.3921, 29.09945), 1, 48, "Клуб Bahama", 255, 0, true); // Блип клуб багама
                NAPI.Blip.CreateBlip(121, new Vector3(141.3792, -1281.576, 28.2172), 1, 48, "Клуб Vanila", 255, 0, true);
               // NAPI.Blip.CreateBlip(136, new Vector3(-564.5512, 275.6993, 81.98249), 1, 48, "Клуб Tequi-la-la", 255, 0, true);
                NAPI.Blip.CreateBlip(102, new Vector3(-430.1028, 261.2774, 81.88689), 1, 4, "Клуб Comedy Club", 255, 0, true);
                NAPI.Blip.CreateBlip(614, new Vector3(-18.94, 239.19, 109.55), 1, 4, "Клуб Galaxy", 255, 0, true);
                ...
                foreach (var pair in ExitAlcoShop)
                {
                    ...
                    NAPI.Marker.CreateMarker(1, pair.Value - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1, new Color(255, 255, 255, 220), false, NAPI.GlobalDimension);
                    NAPI.TextLabel.CreateTextLabel($"~r~Выход", pair.Value + new Vector3(0, 0, 0.5), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                }
                #endregion
                #region Unloadpoints
                foreach (var pair in UnloadPoints)
                {
                    ...
                    NAPI.TextLabel.CreateTextLabel($"~o~Выгрузить все материалы", pair.Value + new Vector3(0, 0, 0.5), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                }
                #endregion
                #region BuyPoints
                foreach (var pair in BuyPoints)
                {
                    ...
                    NAPI.Marker.CreateMarker(1, pair.Value - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1, new Color(255, 255, 255, 220), false, NAPI.GlobalDimension);
                    NAPI.TextLabel.CreateTextLabel($"~b~Купить Алкоголь", pair.Value + new Vector3(0, 0, 0.5), 5f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                }
                #endregion
            }
        ...
        internal class Stock
        {
            ...
            {
                Label = NAPI.TextLabel.CreateTextLabel($"~r~Материалы: {mats}\nAlcohol: {a1 + a2 + a3}", pos, 30f, 0.3f, 0, new Color());
            ...
            {
                Label.Text = $"~r~Материалы: {Materials}\nAlcohol: {Alco1 + Alco2 + Alco3}";
            }
        }
    }
}
