...
namespace NeptuneEvo.Fractions
{
    class Merryweather : Script
    {
        ...
        {
            new Vector3(2477.00, -384.07, 93.40), // Колшэйп входа в бункер
            ...
            new Vector3(2476.67, -388.03, 93.40), // Колшэйп на верх к вертолёту 1
            new Vector3(2506.03, -422.68, 117.03), // Колшэйп изнутри этажа, чтобы вернуться назад
            new Vector3(2477.20, -379.96, 93.40), // Колшэйп на верх к вертолёту 2
            new Vector3(2506.19, -346.93, 117.03), // Колшэйп изнутри этажа, чтобы вернуться назад
        };
        ...
        public void OnResourceStartHandler()
        {
            try
            {
                ...
                Cols.Add(6, NAPI.ColShape.CreateCylinderColShape(Coords[6], 1f, 2, 0));
                Cols[6].OnEntityEnterColShape += mws_OnEntityEnterColShape;
                Cols[6].OnEntityExitColShape += mws_OnEntityExitColShape;
                Cols[6].SetData("INTERACT", 88);

                Cols.Add(7, NAPI.ColShape.CreateCylinderColShape(Coords[7], 1f, 2, 0));
                Cols[7].OnEntityEnterColShape += mws_OnEntityEnterColShape;
                Cols[7].OnEntityExitColShape += mws_OnEntityExitColShape;
                Cols[7].SetData("INTERACT", 89);
                ...
                NAPI.Marker.CreateMarker(1, Coords[7] - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1, new Color(255, 255, 255, 220));
            } catch(Exception e)
            {
        ...
        public static void interactPressed(Client player, int interact)
        {
            switch (interact)
            {
                ...
                case 89:
                    ...
                    {
                        Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Вы не устроены в Наёмниках", 3000);
                        return;
                    }
                    ...
                    else if (interact == 86) NAPI.Entity.SetEntityPosition(player, Coords[5] + new Vector3(0, 0, 1.12));
                    else if(interact == 87) NAPI.Entity.SetEntityPosition(player, Coords[4] + new Vector3(0, 0, 1.12));
                    else if (interact == 88) NAPI.Entity.SetEntityPosition(player, Coords[7] + new Vector3(0, 0, 1.12));
                    else if (interact == 89) NAPI.Entity.SetEntityPosition(player, Coords[6] + new Vector3(0, 0, 1.12));
                    return;
            }
        }
    }
