...
namespace NeptuneEvo.Core
{
    class BusinessManager : Script
    {
	   ...
	   { "bmwe38", 10 }, //Lux salon
        ...
        public static void interactionPressed(Client player)
        {
            ...
            if (biz.Owner != "Гос." && !Main.PlayerNames.ContainsValue(biz.Owner))
            {
        ...
        public static void createBusinessCommand(Client player, int govPrice, int type)
        {
            ...
            MySQL.Query($"INSERT INTO businesses (id, owner, sellprice, type, products, enterpoint, unloadpoint, money, mafia, orders) " +
                $"VALUES ({lastBizID}, 'Гос.', {govPrice}, {type}, '{productlist}', '{JsonConvert.SerializeObject(pos)}', '{JsonConvert.SerializeObject(new Vector3())}', {bankID}, -1, '{JsonConvert.SerializeObject(new List<Order>())}')");

            Business biz = new Business(lastBizID, "Гос.", govPrice, type, products_list, pos, new Vector3(), bankID, -1, new List<Order>());
            biz.UpdateLabel();
    ...
    public class Business
    {
        ...
        {
            string text = $"~y~{BusinessManager.BusinessTypeNames[Type]}\n~d~Владелец: ~w~{Owner}\n";
            if (Owner != "Гос.") text += $"~d~ID: ~w~{ID}\n";
            else text += $"~d~Цена: ~w~{SellPrice}$\n~w~ID: ~w~{ID}\n";
            if (Type == 1)
            {
                text += $"~w~Price for 1l: {Products[0].Price}$\n";
                text += "~w~Press E\n";
            }
            label.Text = Main.StringToU16(text);

            if (Mafia != -1) mafiaLabel.Text = $"~r~Крыша: ~w~{Fractions.Manager.getName(Mafia)}";
            else mafiaLabel.Text = "~r~Крыша: ~w~none";
        }
