...
namespace NeptuneEvo.Core
{
    class Admin : Script
    {
        ...
        public static void sendGC(Client player, Client target, int amount)
        {
            if (!Group.CanUseCmd(player, "givereds")) return;

            if (Main.Accounts[target].GC + amount < 0) amount = 0;
            Main.Accounts[target].GC += amount;
            Trigger.ClientEvent(target, "starset", Main.Accounts[target].GC);

            Notify.Send(player, NotifyType.Success, NotifyPosition.BottomCenter, $"Вы отправили {target.Name} {amount} GC", 3000);
            Notify.Send(target, NotifyType.Success, NotifyPosition.BottomCenter, $"+{amount} GC", 3000);
