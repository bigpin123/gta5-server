    ...
    class Commands : Script
    {
        ...
        public static void CMD_hidenick(Client player) {
            ...
            if (!player.HasSharedData("HideNick") || !player.GetSharedData("HideNick"))
                {
                    player.SendChatMessage("~o~Ники Включены");
                    player.SetSharedData("HideNick", true);
                }
                else
                {
                    player.SendChatMessage("~o~Ники Выключены");
        ...
        public static void CMD_givereds(Client player, int id, int amount)
        {
            try
            {
                ...
                Admin.sendGC(player, target, amount);
            }
            catch (Exception e) { Log.Write("EXCEPTION AT \"CMD\":\n" + e.ToString(), nLog.Type.Error); }
        }
        [Command("checkprop")]
        ...
        {
            if (!Group.CanUseCmd(client, "offGC")) return;
            try
            {
                name = name.ToLower();
                ...
                MySQL.Query($"update `accounts` set `GC`=`GC`+{amount} where `login`='{name}'");
