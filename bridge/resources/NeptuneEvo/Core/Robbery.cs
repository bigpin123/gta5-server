...
namespace NeptuneEvo.Core
{
    public class SafeMain : Script
    {
        ...
        public void SafeCracker_Init()
        {
            try
            {
                for (int b = 0; b < moneyFlowPoints.Count; b++)
                {
                    ...
                    var flowBlip = NAPI.Blip.CreateBlip(514, moneyFlowPoints[b], 1, 85, Main.StringToU16("Черный рынок")); //Блип чёрный рынок
                    NAPI.Entity.SetEntityDimension(flowBlip, 0);
                    flowBlip.ShortRange = true;

                    NAPI.TextLabel.CreateTextLabel(Main.StringToU16($"E\n~r~{moneyFlowers[b]}"), moneyFlowPoints[b] + new Vector3(0, 0, 1.125), 5F, 0.8F, 0, new Color(255, 255, 255));
                }
        ...
        public static void interactSafe(Client player)
        {
            ...
            else
            {
                ...
                {
                    Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, $"Возможно открыть только с 8:00 до 23:00", 3000);
                    return;
                }
        ...
        public static void MoneyFlow(Client player)
        {
            ...
            {
                NAPI.Chat.SendChatMessageToPlayer(player, "~r~[Сергей Мавроди] ~w~Чё ты тут забыл?");
                return;
            }
            var all_money = 0;
    ...
    public class Safe
    {
        ...
        public void Create()
        {
            ...
            Label = NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~o~Сейф"), Position + new Vector3(0, 0, 1.05), 5f, 0.65f, 0, new Color(255, 255, 255), false);
