    ...
    public class Account : AccountData
    {
        ...
        public async Task<RegisterEvent> Register(Client client, string login_, string pass_, string email_, string promo_)
        {
            try
            {
                ...
                HWID = client.Serial;
                if(client.Address.Equals("")) IP = "";
                else IP = client.Address;
                SocialClub = client.SocialClubName;
                await MySQL.QueryAsync($"INSERT INTO `accounts` (`login`,`email`,`password`,`hwid`,`ip`,`socialclub`,`GC`,`viplvl`,`vipdate`,`promocodes`,`character1`,`character2`,`character3`) " +
        ...
        public async Task<bool> Save(Client player)
        {
            try
            {
                ...
                cmd.CommandText = "UPDATE `accounts` SET `password`=@pass,`email`=@email,`socialclub`=@sc,`GC`=@red,`viplvl`=@vipl,`hwid`=@hwid,`ip`=@ip," +
                ...
                cmd.Parameters.AddWithValue("@red", GC);
        ...
        public async Task DeleteCharacter(Client player, int slot, string firstName_, string lastName_, string password_)
        {
            ...
            foreach (var b in biz)
                BusinessManager.changeOwner($"{firstName}_{lastName}", "Гос.");
