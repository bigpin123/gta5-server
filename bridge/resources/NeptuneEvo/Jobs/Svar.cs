...
namespace NeptuneEvo.Jobs
{
    class Svar : Script
    {
        private static int checkpointPayment = 22;
        private static nLog Log = new nLog("Svar");
        ...
        public void Event_ResourceStart()
        {
            try
            {
                NAPI.TextLabel.CreateTextLabel("~r~Прораб сварщик", new Vector3(139.11, -382.22, 44.26), 30f, 0.3f, 0, new Color(255, 255, 255), true, NAPI.GlobalDimension);
                ...
                var col = NAPI.ColShape.CreateCylinderColShape(new Vector3(139.11, -382.22, 42.26), 1, 2, 0);
                col.OnEntityEnterColShape += (shape, player) => {
                    try
                    {
                        player.SetData("INTERACTIONCHECK", 650); // Выставляет номер  650 ставить из кейса
                    }
                ...
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("~o~Начать/Закончить рабочий день"), new Vector3(139.11, -382.22, 45.26), 30f, 0.4f, 0, new Color(255, 255, 255), true, 0);
                NAPI.Marker.CreateMarker(1, new Vector3(139.11, -382.22, 42.26) - new Vector3(0, 0, 0.7), new Vector3(), new Vector3(), 1, new Color(255, 255, 255, 220));
        ...
        public static void StartWorkDay(Client player)
        {
            ...
            {
                Notify.Send(player, NotifyType.Error, NotifyPosition.BottomCenter, "Сначала устройся в мэрии", 3000);
                return;
            }
        ...
        {
            new Checkpoint(new Vector3(88.74, -361.53, 41.17), 252.41),
            new Checkpoint(new Vector3(84.45, -347.78, 41.18), 339.07),
            new Checkpoint(new Vector3(66.35, -333.20, 41.82), 194.79),
            new Checkpoint(new Vector3(47.43, -346.49, 41.52), 255.29),
            new Checkpoint(new Vector3(38.05, -349.88, 41.52), 143.35),
            new Checkpoint(new Vector3(50.20, -360.40, 41.52), 168.40),
            new Checkpoint(new Vector3(42.56, -357.35, 41.52), 149.84),
            new Checkpoint(new Vector3(49.93, -350.41, 41.52), 350.65),
            new Checkpoint(new Vector3(91.56, -346.75, 41.22), 49.78),// чек пойинт места где варить
        ...
        private static void PlayerEnterCheckpoint(ColShape shape, Client player)
        {
            try
            {
                ...
                if (Main.Players[player].WorkID != 9 || !player.GetData("ON_WORK") || shape.GetData("NUMBER") != player.GetData("WORKCHECK")) return;
                ...
                GameLog.Money($"server", $"player({Main.Players[player].UUID})", payment, $"svarCheck");
                ...
                player.PlayAnimation("missmechanic", "work2_in", 39);
