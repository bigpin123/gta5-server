        ...
        public static void callMechanic(Client player)
        {
            if (!NAPI.Data.GetEntityData(player, "IS_CALL_MECHANIC"))
            {
                ...
                foreach (var p in players)
                {
                    ...
                    {
                        i++;
                        NAPI.Chat.SendChatMessageToPlayer(p, $"~o~[ДИСПЕТЧЕР]: ~w~Игрок ({player.Value}) вызвал автомеханика ~y~({player.Position.DistanceTo(p.Position)}м)~w~. Напишите ~y~/ma ~b~[ID]~w~, чтобы принять вызов");
                    }
                }
                if (i > 0)
                {
