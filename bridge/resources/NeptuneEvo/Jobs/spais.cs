...
namespace NeptuneEvo.Jobs
{
    class spais : Script
    {
        private static nLog Log = new nLog("spais");
        ...
        public void onResourceStart()
        {
            try
            {
                ...
                Cols[0].SetData("INTERACT", 550);
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("РАБОЧИЙ ДЕНЬ"), Coords[0] + new Vector3(0, 0, 0.3), 10F, 0.6F, 0, new Color(0, 180, 0));
                ...
                Cols[1].SetData("INTERACT", 551);
                NAPI.TextLabel.CreateTextLabel(Main.StringToU16("Взять Т/С"), Coords[1] + new Vector3(0, 0, 0.3), 10F, 0.6F, 0, new Color(0, 180, 0));
        ...
        {
            new Vector3(-15.22, -1310.12, 29.27), // start work
            new Vector3(-20.38, -1306.10, 29.27), // get car
        };
        private static Dictionary<int, ColShape> Cols = new Dictionary<int, ColShape>();
        private static Dictionary<int, ColShape> gCols = new Dictionary<int, ColShape>();
        // Postal items (objects) //
        public static List<uint> spaisObjects = new List<uint>
        {
            NAPI.Util.GetHashKey("prop_weed_bottle"), //тюбик спайса в руках
        };
        ...
        public static void onPlayerDisconnected(Client player, DisconnectionType type, string reason)
        {
            try
            {
                ...
                if (Main.Players[player].WorkID != 10) return;
        ...
        public void onPlayerExitVehicle(Client player, Vehicle vehicle)
        {
            try
            {
                ...
                {
                    int x = WorkManager.rnd.Next(0, spaisObjects.Count);
                    BasicSync.AttachObjectToPlayer(player, spaisObjects[x], 60309, new Vector3(0.03, 0, 0.02), new Vector3(0, 0, 50));
                }
        ...
        public static void Event_PlayerDeath(Client player, Client entityKiller, uint weapon)
        {
            ...
            {
                if (!Main.Players.ContainsKey(player)) return;
                if (Main.Players[player].WorkID == 10 && NAPI.Data.GetEntityData(player, "ON_WORK"))
                {
        ...
        public static void spais_onEntityEnterColShape(ColShape shape, Client player)
        {
            try
            {
                if (!Main.Players.ContainsKey(player)) return;
                if (HouseManager.Houses.Count == 0) return;
                if (Main.Players[player].WorkID != 10 || !NAPI.Data.GetEntityData(player, "ON_WORK")) return;
                if (player.HasData("NEXTHOUSE") && player.HasData("HOUSEID") && NAPI.Data.GetEntityData(player, "NEXTHOUSE") == player.GetData("HOUSEID"))
                {
                    ...
                    else if (player.GetData("PACKAGES") > 1)
                    {
                        ...
                        Notify.Send(player, NotifyType.Info, NotifyPosition.BottomCenter, $"У Вас осталось {player.GetData("PACKAGES")} закладок", 3000);
                    ...
                    else
                    {
                        ...
                        Notify.Send(player, NotifyType.Warning, NotifyPosition.BottomCenter, $"У Вас не осталось закладок, возьмите новые", 3000);
                    }
                }
            }
            catch (Exception e) { Log.Write("EXCEPTION AT \"spais\":\n" + e.ToString(), nLog.Type.Error); }
        }
        ...
        public static void getspaisCar(Client player)
        {
            ...
            var veh = API.Shared.CreateVehicle(VehicleHash.Faction3, player.Position + new Vector3(0, 0, 1), player.Rotation.Z, 1, 1); // 1 1 цвет авто
