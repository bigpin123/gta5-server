...
namespace NeptuneEvo.Houses
{
    ...
    class House
    {
        ...
        public House(int id, string owner, int type, Vector3 position, int price, bool locked, int garageID, int bank, List<string> roommates)
        {
            ...
            {
                blip = NAPI.Blip.CreateBlip(Position); // Блип домов
            ...
            shape.OnEntityEnterColShape += (s, ent) =>
            {
                try
                {
                    NAPI.Data.SetEntityData(ent, "HOUSEID", id);
                    NAPI.Data.SetEntityData(ent, "INTERACTIONCHECK", 6);
                    Jobs.spais.spais_onEntityEnterColShape(s, ent);
        ...
        public void UpdateLabel()
        {
            try
            {
                string text = $"~y~Кл.: ~w~{HouseManager.HouseTypeList[Type].Name}\n";
                if (!string.IsNullOrEmpty(Owner)) text += $"~w~Owner: ~w~{Owner.Replace('_', ' ')}\n";
                else text += $"~d~Цена ~w~{Price}$\n";
                if (GarageID != 0) text += $"~d~Г.мест: ~w~{GarageManager.GarageTypes[GarageManager.Garages[GarageID].Type].MaxCars}\n";
                text += (Locked) ? "~r~Закрыт\n" : "~g~Открыт\n";
                text += $"~d~ID: ~w~{ID}";
    ...
    class HouseManager : Script
    {
        ...
        {
            // name, position
            new HouseType("Трейл", new Vector3(1973.124, 3816.065, 32.30873), null, 0.0f, "trevorstrailer"),
            new HouseType("Экон", new Vector3(151.2052, -1008.007, -100.12), null, 0.0f,"hei_hw1_blimp_interior_v_motel_mp_milo_"),
            new HouseType("Экон+", new Vector3(265.9691, -1007.078, -102.0758), null, 0.0f,"hei_hw1_blimp_interior_v_studio_lo_milo_"),
            new HouseType("Комф", new Vector3(346.6991, -1013.023, -100.3162), new Vector3(349.5223, -994.5601, -99.7562), 264.0f, "hei_hw1_blimp_interior_v_apart_midspaz_milo_"),
            new HouseType("Комф+", new Vector3(-31.35483, -594.9686, 78.9109),  new Vector3(-25.42115, -581.4933, 79.12776), 159.84f, "hei_hw1_blimp_interior_32_dlc_apart_high2_new_milo_"),
            new HouseType("Прем", new Vector3(-17.85757, -589.0983, 88.99482), new Vector3(-38.84652, -578.466, 88.58952), 50.8f, "hei_hw1_blimp_interior_10_dlc_apart_high_new_milo_"),
            new HouseType("Прем+", new Vector3(-173.9419, 497.8622, 136.5341), new Vector3(-164.9799, 480.7568, 137.1526), 40.0f, "apa_ch2_05e_interior_0_v_mp_stilts_b_milo_"),
        };
